...
Accepted input is durable before a Discord handler returns. When delivery is ready, the adapter adds 📥 to the source message after saving it. 👀 appears only after the bound native owner records a message-specific acknowledgment with the exact native ID and generation. Neither signal means completion. Missing 👀 means recognition is unverified, not that the agent is idle. Failed reactions do not undo intake or delay native work.

When delivery was paused, the transport keeps the short text reply `Receipt: saved. Delivery was paused when this receipt was prepared.` Receipt attempts remain independent of native forwarding and never retry an uncertain send. Duplicate or rejected input gets no new saved-receipt attempt.
...
- [Roadmap and acceptance summary](docs/spark-sidecar-roadmap.md)
