...
There is no Discord-side compare-and-swap for a topic. If a handoff commits while a topic request is in flight, the adapter fences the late completion from local readiness and successor custody. A remote late request can leave the old marker visible until explicit handoff topic repair. Successor recovery rejects that old marker, so it cannot authorize the successor.
