...
const { CODEX_VALIDATION_KINDS, codexPrompt, dispatchAndObserve, ClaudeProvider, CodexProvider, observeSubmitted, probeClaudeChannel, readInitialCursor, validateCodexSessionIdentity, validateCodexSessionIdentityAsync, waitForReply } = require('./native');
...
function createSurfaceConsumer({ state, providers, sendReply, sendTransportReceipt, prepareReply, trackReceipt, observeOptions = {},
  ...
  async function dispatchAtCourierBoundary(message, _parentProvider, dispatchOptions, selected) {
    ...
    const prompt = codexPrompt(message, acknowledgmentCommand(message, state.dbPath));
...
class DiscordGateway {
  constructor({ state, client, logger = () => {}, observeOptions = {}, providers, fetchHistory, recoveryOptions = {}, onReady = null, interactionFetch = globalThis.fetch, courierRoute = null } = {}) {
    this.state = state;
    this.logger = logger;
