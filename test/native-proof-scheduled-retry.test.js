...
test('startup keeps a native proof deadline marker until a later lifecycle wake', { timeout: 5000 }, async t => {
  ...
  await gateway.beginReconnectRecovery('test lifecycle wake');
  assert.equal(state.getIntakeWatermark('1000').state, 'ready');
  assert.equal(preflights, 2);
});

test('an explicit binding wake retries a native proof deadline marker', { timeout: 5000 }, async t => {
...
test('Claude endpoint recovery does not invent a native proof wake', { timeout: 5000 }, async t => {
  ...
  assert.equal(gateway.deferredHandoffRecoveryChannels.has('1000'), false);
