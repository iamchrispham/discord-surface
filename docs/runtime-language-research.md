...
Five internal lenses considered mechanism, counterarguments, evidence, economics and measurement limits. Three completed external Pro rounds are summarized in this decision and the [runtime memory baseline](runtime-memory-direction.md). Pro first favored optimized Node with Go as challenger, then Go as the intended foundation. Its final answer withdrew Go-first because simpler maintenance and iteration were unmeasured expectations for this repository. Model agreement is not performance evidence.
