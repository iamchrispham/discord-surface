...
The feature candidate now emits disabled skill names from frontmatter, includes symlinked directories and deduplicates names. Global skill files and account configuration remain unchanged. A focused test exercises the actual command builder with directory, symlink, duplicate and broken-link fixtures. The old builder fails the exact name-selector assertion; the corrected builder passes. The package command ran both registered test files serially: 124 passed, 0 failed, 0 skipped. An independent read-only review found no blocking issue. This correction has not been loaded into the production Gateway, and full skill isolation remains unproved.
