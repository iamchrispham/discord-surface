...
September 6 publication implementation: `src/publication/` now owns automatic deterministic posting inside the Gateway, with source file events, fixed burst coalescing, cooldown, expiry, separate processed/successful cursors, uncertain-send custody and restart-safe publication identity exclusion in live and history intake. Status exposes publication state counts. Companion checks exercise actual file replacement and SQLite reopen, including a bot-flag-stripped replay. This is not installed. Contextual publication and a bounded reference for human replies remain required. See `docs/publication-proof.md`; S0–S5 completion boxes remain open pending their full proof scope.

## Conductor milestones and notification migration

Operator ruling, September 6: unprompted conductor communications are milestones only: a landing, a blocker, or a ruling the operator may want to override. Round-by-round details stay in the conductor beacon and PR bodies. Human-grade events still page the phone. The missing unprompted adapter command is an implementation gap, not an operator restriction and not a Spark/Luna dependency.

- [ ] Provide a conductor-owned milestone post command that does not require an inbound Discord message. Preserve the existing native identity, current binding generation and durable delivery custody. Explicit milestone messages must not be superseded by the automatic board's latest-snapshot coalescing.
- [ ] Prove a conductor can deliver a milestone into its bound channel without an operator prompt or sidecar inference. Ordinary automatic summaries do not authorize additional conductor turns or round-by-round channel chatter.
- [ ] Retire routine Telegram notifications after the corresponding Discord route has actual delivery proof. Choose one destination for each notification class and disable the old sender during cutover. Do not dual-post as a steady state.
- [ ] Retire Telegram-based human paging separately, only after a replacement proves actionable notification, response correlation and durable receipt. Until that proof and cutover, retain the existing phone path for human-grade events. This roadmap entry does not disable it.

The accepted destination policy is one route per notification class. If delivery is uncertain, preserve its custody and reconcile it before any alternate delivery, rather than blindly posting the same notification to a second surface. The exact replacement for phone paging remains future work.


September 6 direct milestone delivery: isolated PR 3, source `ec2414559c675ffd866fe7a476a50632746e3116`, implements `post` and `claude-post` using existing receipt custody, binding generation checks, newline-aware splitting and no new schema. The isolated owning suite passed 131 tests with 0 failures/skips. Independent review clear after a trailing-newline correction. Three negative controls each caught an actual second network request. A real Codex milestone was fetched by Discord message ID `1546097923077447851`. Exact command repeat reused its receipt with one attempt and one outcome. Discord removed the file's final newline, so byte-identical server whitespace is not claimed. CLI candidate: `outputs/discord-post-release/src/cli.js`. Gateway 87202 and TM Monitor 36742 remained running, no sidecar schema was added, and no phone route changed. TM's exact command and current generation were durably queued in `outputs/discord-tm-post-handoff.md`; queue acceptance is not owner execution proof. See `outputs/conductor-post-proof.json`.

September 6 combined publication and milestone integration: all five registered suites passed 164 tests after independent review caught and corrected a missing wakeup following bot-echo settlement of uncertain custody. A real-file/SQLite regression proves the newer pending snapshot resumes automatically under the normal cooldown. Removing the event fails that regression. The PR 3 trailing-newline and direct-post preflight corrections are included. This remains an uninstalled candidate. Contextual publication, bounded human references and live both-provider acceptance remain open.
