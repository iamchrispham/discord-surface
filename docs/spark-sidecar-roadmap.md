...
Local inspection corrected two supplied assumptions: `lane-progress.py` does not yet skip underscore dictionaries, and `round-close.sh` writes head/trigger/round/optional phase but not next/state_note/updated. TM owns shared reader compatibility and producer rollout through its normal skills workflow. Consultation was queued, not yet acknowledgment-proven. Snapshot prototype reads the corrected shape and leaves absent fields unknown. It is not an automatic publisher.
